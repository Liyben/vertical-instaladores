            ...
            for task in self:
                if self.env.user in task.user_ids:
                    ...
                    if employee:
                        ...
                        if attendance:
                            aal_start = self.env['account.analytic.line'].search([('attendance_id', '=', attendance.id), ('employee_id', '=', employee.id)])
                            if aal_start:
                                aal_stop = self.env['account.analytic.line'].search([('attendance_id', '=', attendance.id), ('employee_id', '=', employee.id), ('task_id', '=', task.id)])
                                if aal_stop:
                                    task.show_time_control = "stop"
                                else:
