...
class StockInvoiceOnshipping(models.TransientModel):
	...
	journal_type_for_action = fields.Selection(
		selection=[
	...
	def action_generate(self):
		...
		inv_type = self._get_invoice_type_for_action()
	...
	def _load_pickings(self):
		...
		# Ponemos el pedido de compra/venta como facturado
	...
	@api.multi
	def _get_invoice_type_for_action(self):
		"""
		Get the invoice type
		:return: str
		"""
		...
		inv_type = journal2type.get(self.journal_type_for_action) or 'out_invoice'
		return inv_type
	...
	@api.multi
	def _get_invoice_for_type(self, picking):
		"""
		Obtenemos el tipo de fatura según los datos
		del albaran
		:return: str
		"""
		...
		loc_dst = picking.location_dest_id
		picking_type = picking.picking_type_code
		if picking_type == 'outgoing' and loc.usage == 'customer' and loc_dst.usage == 'internal':
			inv_type = 'out_refund'
			journal_type = 'sale_refund'
		elif picking_type == 'outgoing' and loc.usage == 'internal' and loc_dst.usage == 'customer':
			inv_type = 'out_invoice'
			journal_type = 'sale'
		elif picking_type == 'incoming' and loc.usage == 'supplier' and loc_dst.usage == 'internal':
			inv_type = 'in_invoice'
			journal_type = 'purchase'
		elif picking_type == 'incoming' and loc.usage == 'internal' and loc_dst.usage == 'supplier':
			inv_type = 'in_refund'
			journal_type = 'purchase_refund'
		self.journal_type_for_action = journal_type
		return inv_type
	...
	def _build_invoice_values_from_pickings(self, pickings):
		...
		inv_type = self._get_invoice_for_type(picking)
		if inv_type in ('out_invoice', 'out_refund'):
		...
		for pickings in pick_list:
			...
			for moves_list in parts:
				...
				if line_values:  # Only create the invoice if it have lines
					...
					invoices |= invoice
		"""Añadimos las facturas al pedido de compra/venta
		for pickings in pick_list:
			pickings.set_invoices_to_sale(invoices)
			pickings.set_invoices_to_purchase(invoices)"""
		return invoices
