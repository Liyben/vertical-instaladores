...
class SaleOrderLine(models.Model):
	...
	sections_ids = fields.One2many(
		comodel_name = 'sale.order.line.sections',
	...
	#Opciones de impresión para las secciones
	detailed_sections = fields.Boolean(string='Imp. secciones')

	""" last_seccion_name = fields.Char(string="Ultima sección",compute = '_compute_last_section_name',store=True) """
	...
	@api.depends('sections_ids', 'sections_ids.ancho', 
				'sections_ids.alto', 'sections_ids.mts_cuadrados',
				'sections_ids.unidades', 'sections_ids.mts_cuadrados_sub')
	def _compute_total_sections(self):
		for line in self:
			line.total_lineales = sum(line.sections_ids.mapped('mts_lineales_sub'))
			line.total_cuadrados = sum(line.sections_ids.mapped('mts_cuadrados_sub'))

	""" @api.depends('sections_ids','sections_ids.seccion_name')
	def _compute_last_seccion_name(self):
		for line in self:
			if len(line.sections_ids) == 0:
				line.last_seccion_name = 'A'
			else:
				s_name = line.sections_ids[len(line.sections_ids)-1].seccion_name
...
class SaleOrderLineSectionsType(models.Model):
    _name = 'sale.order.line.sections.type'
    _description = 'Tipo de secciones'

    name = fields.Char('Nombre de sección', required=True, translate=True)
    
class SaleOrderLineSections(models.Model):
	
	_name = 'sale.order.line.sections'
	...
	section_type_id = fields.Many2one(
		comodel_name = 'sale.order.line.sections.type', 
		string = 'Sección', 
		required = True,
	)
